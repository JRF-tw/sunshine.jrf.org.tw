class CourtObservers::SchedulesController < CourtObservers::BaseController
  ...
  def schedule_score_params
    params.fetch(:schedule_score, {}).permit(:id, :court_id, :year, :word_type, :number, :start_on, :confirmed_realdate, :judge_name, :rating_score, :note, :appeal_judge)
  end
  ...
  def find_schedule_score
    ...
    redirect_as_fail(court_observer_stories_path, "沒有該評鑑紀錄") unless @schedule_score
  end

  def story_adjudged?
    redirect_as_fail(court_observer_stories_path, "案件已判決") if @schedule_score.story.adjudge_date.present?
  end
end
