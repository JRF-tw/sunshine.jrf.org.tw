class Admin::CourtsController < Admin::BaseController
  ...
  def edit
    @admin_page_title = "編輯法院 - #{court.name}"
    add_crumb @admin_page_title, '#'
  end
  ...
  def create
    ...
    if @court = context.perform
      respond_to do |f|
        f.html { redirect_as_success(admin_courts_path, "法院 - #{court.name} 已新增") }
        f.js { render }
      end
    else
      @court = context.court
  ...
  def update
    ...
    if context.perform(params)
      redirect_as_success(admin_courts_path, "法院 - #{court.name} 已修改")
    else
  ...
  def destroy
    ...
    if context.perform
      redirect_as_success(admin_courts_path, "法院 - #{court.name} 已刪除")
    else
