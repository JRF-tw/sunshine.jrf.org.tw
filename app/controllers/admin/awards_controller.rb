class Admin::AwardsController < Admin::BaseController
  ...
  before_action(except: [:index]) { add_crumb("#{@owner.name}的獎勵紀錄列表", send("admin_#{owner_singularize}_awards_path", @owner.id)) }
  ...
  def create
    if award.save
      respond_to do |f|
        f.html { redirect_to send("admin_#{owner_singularize}_awards_path", @owner), flash: { success: "#{@owner.name}的獎勵紀錄 - 已新增" } }
        f.js { render }
      end
    else
      respond_to do |f|
  ...
  def update
    if award.update_attributes(award_params)
      redirect_to send("admin_#{owner_singularize}_awards_path", @owner), flash: { success: "#{@owner.name}的獎勵紀錄 - 已修改" }
    else
      @admin_page_title = "編輯#{@owner.name}的獎勵紀錄"
      add_crumb @admin_page_title, '#'
      flash[:error] = award.errors.full_messages
  ...
  def destroy
    if award.destroy
      redirect_to send("admin_#{owner_singularize}_awards_path", @owner), flash: { success: "#{@owner.name}的獎勵紀錄 - 已刪除" }
    else
      flash[:error] = award.errors.full_messages
      redirect_to :back
    end
