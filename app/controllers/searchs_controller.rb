class SearchsController < BaseController
  ...
  def judges
    ...
    @people = Profile.judges.shown.find_current_court(params_utf8[:judge]).front_like_search({ :name => params_utf8[:q] }).page(params[:page]).per(12)
    set_meta(
      title: "法官搜尋結果",
  ...
  def prosecutors
    ...
    @people = Profile.prosecutors.shown.find_current_court(params_utf8[:prosecutor]).front_like_search({ :name => params_utf8[:q] }).page(params[:page]).per(12)
    set_meta(
      title: "檢察官搜尋結果",
