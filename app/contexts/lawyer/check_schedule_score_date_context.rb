class Lawyer::CheckScheduleScoreDateContext < BaseContext
  ...
  SCORE_INTERVEL = BaseContext::SCHEDULE_SCORE_INTERVEL
  MAX_REPORT_TIME = 5
  ...
  def record_report_time
    @lawyer.score_report_schedule_real_date.increment
  end

  def alert!
    if @lawyer.score_report_schedule_real_date.value >= MAX_REPORT_TIME
      SlackService.notify_report_schedule_date_over_range_alert("律師 : #{@lawyer.name} 已超過回報庭期真實開庭日期次數")
    end
  end
