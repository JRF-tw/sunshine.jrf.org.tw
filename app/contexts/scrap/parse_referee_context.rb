class Scrap::ParseRefereeContext < BaseContext
  ...
  before_perform :parse_referee_adjudge_date
  ...
  def perform
    run_callbacks :perform do
      if referee_type == 'verdict'
        Scrap::ImportVerdictContext.delay(retry: false, queue: 'crawler_verdict').perform(@court, @orginal_data, @referee_content, @referee_word, @referee_adjudge_date, @referee_story_type)
      else
        Scrap::ImportRuleContext.delay(retry: false, queue: 'crawler_rule').perform(@court, @orginal_data, @referee_content, @referee_word, @referee_adjudge_date, @referee_story_type)
      end
    end
  end
  ...
  def parse_referee_adjudge_date
    date_string = @nokogiri_data.css('table')[4].css('tr')[1].css('td')[0].text.match(/\d+/)[0]
    @referee_adjudge_date = Date.new((date_string[0..2].to_i + 1911), date_string[3..4].to_i, date_string[5..6].to_i)
  rescue
