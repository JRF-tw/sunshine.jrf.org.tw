class PartyMailer < ApplicationMailer
  ...
  def after_verdict_notice(verdict_id, party_id)
    verdict = Verdict.find(verdict_id)
    @story = verdict.story
    @party = Party.find(party_id)
    @court = @story.court
    @verdict_url = verdict.file.url
    @limit_date = @story.got_verdict_date + 3.months
  ...
  def generate_score_schedules_link
    input_judge_party_score_schedules_url(
      schedule_score: {
