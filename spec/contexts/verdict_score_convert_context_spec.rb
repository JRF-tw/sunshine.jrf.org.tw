...
describe VerdictScoreConvertContext do
  ...
  context '#perform' do
    context 'party' do
      ...
      context 'verdict with party and judges' do
        let!(:verdict) { create :verdict_for_convert_valid_score, story: story, judges_names: [judge_A.name, judge_B.name], party_names: [role.name] }
    ...
    context 'lawyer' do
      ...
      context 'verdict with lawyer and judges' do
        let!(:verdict) { create :verdict_for_convert_valid_score, story: story, judges_names: [judge_A.name, judge_B.name], lawyer_names: [role.name] }
        it { expect { subject }.to change { ValidScore.count }.by(2) }
      end
    end
