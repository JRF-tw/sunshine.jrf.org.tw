...
RSpec.describe Scrap::ImportSupremeCourtJudgeContext, type: :model do
  describe '#perform' do
    ...
    let(:data_string) { '李錦美(雪' }
    let(:chamber_name) { '民事庭' }
    subject { described_class.new(data_string, chamber_name).perform }

    context 'success' do
      it { expect(subject.name).to eq('李錦美') }
      it { expect(subject.court).to eq(court) }
      it { expect(subject.branches.last.name).to eq('雪') }
      it { expect(subject.branches.last.chamber_name).to eq('民事庭') }
      it { expect { subject }.to change { Judge.count }.by(1) }
    end

    context 'judge exist' do
      let!(:judge) { create :judge, court: court, name: '李錦美' }
