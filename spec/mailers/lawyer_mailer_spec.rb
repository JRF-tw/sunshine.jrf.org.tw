...
RSpec.describe LawyerMailer, type: :mailer do
  ...
  context '#after_verdict_notice' do
    ...
    let(:mail) { described_class.after_verdict_notice(story.verdicts.last.id, lawyer.id).deliver_now }
  ...
  context '#active_verdict_notice' do
    let!(:story) { create :story, :with_adjugde_verdict, :adjudged }
    let(:mail) { described_class.active_verdict_notice(story.verdicts.last.id, lawyer.id).deliver_now }
