module PartyHelper
  ...
  def party_with_unconfirm_phone_number(phone_number)
    party = create(:party)
    signin_party(party)
    post "/party/phone", party_change_phone_form_object: { unconfirmed_phone: phone_number }
    signout_party
    party
  end
  ...
  def party_chang_phone_number_times(times)
    times.times { put "/party/phone", party_change_phone_form_object: { unconfirmed_phone: "09" + rand(10_000_000..99_999_999).to_s } }
  end
