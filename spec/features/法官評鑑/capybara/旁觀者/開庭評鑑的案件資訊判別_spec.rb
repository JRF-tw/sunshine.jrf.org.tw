  ...
  feature "開庭評鑑的案件資訊判別" do
    Scenario "開庭評鑑流程中，正在輸入案件資訊的頁面" do
      ...
      Given "輸入非該法院下的正確案件資訊" do
        before { capybara_court_observer_input_info_schedule_score(story, court: court1) }
        When "送出" do
          before { click_button "下一步" }
      ...
      Given "輸入不存在的案件資訊" do
        ...
        before { capybara_court_observer_input_info_schedule_score(story, word_type: error_word_type) }
        When "送出" do
          before { click_button "下一步" }
          Then "顯示錯誤訊息，頁面仍保留原始輸入資訊" do
