    ...
    feature '案件資訊驗證' do
      Scenario '判決評鑑流程中，正在輸入案件資訊的頁面' do
        ...
        Given '輸入該法院下的正確案件資訊' do
          ...
          When '送出' do
            # capybara_party_input_info_verdict_score should not click
        ...
        Given '輸入非該法院下的正確案件資訊' do
          before { capybara_party_input_info_verdict_score(story, court: court1) }
          When '送出' do
            # capybara_party_input_info_verdict_score should not click
        ...
        Given '輸入不存在的案件資訊' do
          ...
          before { capybara_party_input_info_verdict_score(story, word_type: error_word_type) }
          When '送出' do
            # capybara_party_input_info_verdict_score should not click
            Then '顯示錯誤訊息，頁面仍保留原始輸入資訊' do
