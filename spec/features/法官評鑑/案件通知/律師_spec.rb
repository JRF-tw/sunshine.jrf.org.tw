...
describe '律師案件通知', type: :request do
  ...
  context '取消訂閱' do
    let!(:lawyer) { lawyer_subscribe_story_date_today }
    let(:params) { { token: lawyer.unsubscribe_token } }
    context '案件已訂閱' do
      before { Story::SubscriptionDeleteContext.new(Story.last, lawyer).perform(params) }
    ...
    context '案件未訂閱' do
      ...
      let(:params) { { token: lawyer.unsubscribe_token } }
      subject { Story::SubscriptionDeleteContext.new(story, lawyer).perform(params) }
