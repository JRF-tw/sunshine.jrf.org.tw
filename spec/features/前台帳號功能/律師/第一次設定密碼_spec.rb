...
feature '該律師為了完成註冊，第一次設定密碼', type: :feature, js: true do
  let!(:lawyer) { capybara_build_lawyer }
  ...
  feature '有律師登入的狀態下，就無法進入設定密碼頁' do
    Scenario '該律師已登入（已通過認證)' do
      Given '該律師已認證+登入' do
        ...
        before { capybara_signin_lawyer(email: lawyer.email, password: '00000000') }
    ...
    Scenario '已登入律師B' do
      Given '該律師已收到密碼設定信件, 且律師B已認證+登入' do
        ...
        let!(:lawyer_b) { capybara_build_lawyer(lawyer_b_data) }
        before { capybara_setting_password_lawyer(lawyer_b, password: '11111111') }
        before { capybara_signin_lawyer(email: lawyer_b.email, password: '11111111') }
