      ...
      feature "有律師登入的狀態下，就無法進入設定密碼頁" do
        ...
        Scenario "已登入律師B" do
          Given "該律師已收到密碼設定信件, 且律師B已認證+登入" do
            ...
            let!(:lawyer_b) { build_lawyer(lawyer_b_data) }
            before { setting_password_lawyer(lawyer_b, password: "11111111") }
            before { signin_lawyer(email: lawyer_b.email, password: "11111111") }
      ...
      feature "完成密碼設定後，註冊狀態改為已認證" do
        Scenario "該律師尚未通過認證" do
          Given "該律師已收到密碼設定信件誒, 且前往密碼設定頁" do
            before { visit(edit_lawyer_password_path(reset_password_token: reset_password_token)) }
            When "正確輸入完密碼後送出" do
              before { lawyer_input_set_password_form(password: "123123123") }
