...
feature '觀察者註冊 (Email)', type: :feature, js: true do
  ...
  feature '相同的 email 無法再註冊' do
    Given '已存在觀察者A' do
      ...
      before { input_sign_up_observer(observer_a) }
      before { regiter_submit_observer }
      ...
      When '使用觀察者A的 email 進行註冊' do
        ...
        before { visit(new_court_observer_registration_path) }
        before { input_sign_up_observer(observer_b) }
        before { regiter_submit_observer }
    ...
    Scenario '尚未通過驗證' do
      Given '觀察者A已註冊，但尚未通過驗證' do
        ...
        When '以觀察者A的帳號進行登入' do
          ...
          before { input_sign_in_observer(observer_a) }
          before { log_in_observer }
