...
describe '爬蟲更新法院資料的機制', type: :request do
  context '爬蟲抓到資料(包含法院代號和法院名稱)' do
    ...
    context '爬蟲的法院代號、資料庫有' do
      let(:data_hash) { { scrap_name: '爬蟲名稱', code: '法院代碼' } }
      let!(:court) { create(:court, code: '法院代碼') }
      subject { Scrap::ImportCourtContext.new(data_hash).perform }
      ...
      context '爬蟲名稱和全名不相同時' do
        let!(:court) { create(:court, code: '法院代碼', full_name: '舊全名') }
      ...
      context '爬蟲名稱和簡稱不相同、但全名是相同' do
        let!(:court) { create(:court, code: '法院代碼', name: '舊簡稱', full_name: '爬蟲名稱') }
    ...
    context '資料庫的法院代號、爬蟲沒有' do
      let!(:court) { create(:court, code: '法院代碼', name: '舊簡稱', full_name: '舊全名', scrap_name: '爬蟲名稱') }
      let!(:court1) { create(:court, code: '法院代碼1', name: '舊簡稱', full_name: '舊全名', scrap_name: '爬蟲名稱') }
