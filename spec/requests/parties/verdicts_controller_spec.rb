...
RSpec.describe Parties::VerdictsController, type: :request do
  ...
  context "score flow" do
    ...
    describe "#check_judge" do
      ...
      context "error params" do
        subject! { post "/party/score/verdicts/check_judge", verdict_score: {} }
        it { expect(response).to be_redirect }
        it { expect(flash[:error]).to eq("選擇法院不存在") }
      end
    end
    ...
    describe "#create" do
      context "success" do
        let!(:params) { { court_id: court.id, year: story.year, word_type: story.word_type, number: story.number, story_type: story.story_type, judge_name: judge.name, rating_score: 1, note: "xxxxx", appeal_judge: false } }
