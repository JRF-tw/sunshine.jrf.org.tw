...
RSpec.describe Admin::ScorePresenters do
  ...
  describe '#result' do
    context 'search rater' do
      ...
      it { expect(subject.sorted_score.include?(verdict_score)).to be_truthy }
      it { expect(subject.sorted_score.include?(same_rater_schedule_score)).to be_truthy }
    end

    context 'search score_type' do
      before { params[:score_type_eq] = 'VerdictScore' }
      it { expect(subject.sorted_score.last).to eq(verdict_score) }
    end

    context 'search judge_id' do
      context 'id exist' do
        before { params[:judge_id_eq] = schedule_score.judge_id }
        it { expect(subject.sorted_score.last).to eq(schedule_score) }
      end

      context 'id not_exist' do
        before { params[:judge_id_eq] = schedule_score.judge_id + 10 }
        it { expect(subject.sorted_score).to be_empty }
      end
    end
  end
end
